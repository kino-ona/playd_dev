...
if (is_uploaded_file($img_file['tmp_name'])) {
    ...
    $data_sv_dir = '/'.Y1_DATAFILES_DIR.'/store_pr/'.$ym;
    $data_dir    = Y1_DATAFILES_PATH.'/store_pr/'.$ym;
    ...
    // 이미지 압축 후 업로드
    // $upload = compress_image($fname_tmp, $data_dir.'/'.$fname, 70);
    
    // if (!$upload) {
        // echo "file upload error comp";
        // exit;
    // }
...
// 파일유무
if ($img_url) {
    $sql_file = " pr_img_url = '{$img_url}', ";
    
    // 파일 수정시 기존 파일 삭제
    unlink(Y1_PATH.$store['pr_img_url']);
}
