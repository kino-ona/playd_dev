...
if (is_uploaded_file($img_file['tmp_name'])) {
    ...
    $data_sv_dir = '/'.Y1_DATAFILES_DIR.'/store_icon/'.$ym;
    $data_dir    = Y1_DATAFILES_PATH.'/store_icon/'.$ym;
    ...
    // $thumb_72x72 = thumbnail($fname, $fname_tmp, $data_dir, "72", "72", true, true);
    
    // if (!$thumb_72x72) {
        // echo "file upload error 72";
        // exit;
    // }

    // 썸네일 이미지 72x72
    // $img_url = $data_sv_dir.'/'.$thumb_72x72;
    
    // 업로드가 안된다면 에러메세지 출력하고 die
    if(!move_uploaded_file($fname_tmp, $data_dir.'/'.$fname)) {
        // alert("file upload error 72");
        echo "file upload error 72";
        exit;
    }
...
// 파일유무
if ($img_url) {
    $sql_file = " icon_img_url = '{$img_url}', ";
    
    // 파일 수정시 기존 파일 삭제
    unlink(Y1_PATH.$store['icon_img_url']);
}
