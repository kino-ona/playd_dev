...
if ($m == "") {
    ...
    /*if (!$in['S_SEQ'])
        alert('존재하지 않는 정보입니다.');
	*/
	$sql="select * from   T_POST_CATEGORY  where C_SEQ='".$no."' ";
	$in = sql_fetch($sql);
...
<form name="frm" method="post" action="<?php echo P1_PAGE_URL ?>/post_category_insert.php" onsubmit="return send();" enctype="multipart/form-data">
<input type="hidden" name="m" value="<?php echo $m ?>">
<?php if ($in['C_SEQ']) { ?>
<input type="hidden" name="seq" value="<?=$in['C_SEQ']?>">
<?php } ?>
    <fieldset>
    <legend class="hid1">post 게시판 카테고리 관리</legend>
        ...
        <div class="bothButton">
            <div class="fl">
                <button class="btn btn-inverse" type="button" onclick="fnGoView('<?php echo P1_PAGE_URL ?>/post_category_list.php');">목록</button>
            </div>
            <div class="fr">
                <?php if ($m == "u") { ?>
                <input type="submit" class="btn btn-primary" value="수정">
                <button class="btn btn-inverse" type="button" onclick="deletes('<?php echo P1_PAGE_URL ?>/post_category_del.php');">삭제</button>
                <?php } ?>
                <?php if ($m == "") { ?>
