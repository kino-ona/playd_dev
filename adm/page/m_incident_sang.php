...
$sql_common = " from {$p1['mable_t_support_table']} ";
...
<form name="multi_del_frm" id="multi_del_frm" action="<?php echo P1_PAGE_URL ?>/m_incident_update.php" onsubmit="return multi_del();" method="post">
...
<table class="rowTable">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?=($bnum-$i)?></td>
            <td class="al"><a href="javascript:view('<?=$row['S_SEQ']?>', '<?php echo P1_PAGE_URL ?>/m_incident_edit.php')"><?=$row['S_FIELD']?></a></td>
            <td><?=$row['S_OBJ']?></td>
            <td><?=$row['S_JOB']?></td>
            <td><?=$date_txt?></td>
            <td><a href="javascript:view('<?=$row['S_SEQ']?>', '<?php echo P1_PAGE_URL ?>/m_incident_view.php')" class="btn-small btn-inverse"><?=$row['CNT']?></a></td>
        </tr>
        <?php
        }
        if ($i == 0)
...
<div class="fl">
    ...
    <div class="fr">
        ...
        <?}?>
        <?if($auth['write']=='Y'){?>
        <a href="<?php echo P1_PAGE_URL ?>/m_incident_edit.php?<?=$qstr?>" class="btn btn-primary">등록</a>
        <?}?>
    </div>
</div>
