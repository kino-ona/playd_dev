...
/* 입사지원관리 정보 */
$sql_common_inc = " from {$p1['t_pool_table']} ";
...
<table class="rowTable">
    ...
    <tbody>
        <tr>
            <td class="al"><a href="javascript:view('<?=$row_inc['S_SEQ']?>', '<?php echo P1_PAGE_URL ?>/pl_incident_edit.php')"><?=$row_inc['S_FIELD']?></a></td>
            <td><?=$row_inc['S_OBJ']?></td>
            <td><?=$row_inc['S_JOB']?></td>
            <td><?=$inc_date_txt?></td>
            <td><?=$row_inc['CNT']?></td>
...
<table class="rowTable MT20">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><a href="<?php echo P1_PAGE_URL ?>/download_sptv.php?seq=<?=$row['SV_SEQ']?>&mode=pl" class="btn-small btn-primary">다운</a></td>
            <?php } else { ?>
            <td></td>
            <?php } ?>
        </tr>
...
<div class="bothButton">
    <div class="fr">
        <button class="btn btn-primary" type="button" onclick="fnGoView('<?php echo P1_PAGE_URL ?>/pl_incident.php');">입사지원리스트로</button>
    </div>
</div>
