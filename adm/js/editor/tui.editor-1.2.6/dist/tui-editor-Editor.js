    ...
    value: function _initDefaultImageImporter() {
      this.eventManager.listen('addImageBlobHook', function (blob, callback) {
        ...
        reader.onload = function (event) {
          // event.target.result - 이미지 바이너리
          // blob으로 fwrite.. 파일명은 blob['name'] blob 안에 파일 정보가 들어있음
          // 서버에 파일 업로드후 url 따서 callback에 삽입
          var ext = /^(image\/bmp|image\/gif|image\/jpg|image\/jpeg|image\/png)$/i;
          if(!ext.test(blob.type)) {
            alert('이미지만 가능합니다.');
            return;
          }
          ...
          $.ajax({
            type: 'POST',
            url: '/plugin/tui.editor-1.2.6/file_upload.php',
            data: fd,
            processData: false,
            contentType: false,
          }).done(function(data){
