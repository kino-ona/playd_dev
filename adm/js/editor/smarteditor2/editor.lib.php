...
function editor_html($id, $content, $is_dhtml_editor=true)
{
    ...
    $editor_url = P1_EDITOR_URL.'/'.$board['BC_EDITOR'];
    ...
    if ($is_dhtml_editor && $js) {
        ...
        $html .= "\n".'<script>var editor_url = "'.$editor_url.'", oEditors = [], ed_nonce = "'.ft_nonce_create('smarteditor').'";</script>';
...
if(!function_exists('ft_nonce_create')){
    function ft_nonce_create( $action = '',$user='', $timeoutSeconds=FT_NONCE_DURATION ){
        ...
        $_SESSION['token_'.FT_NONCE_SESSION_KEY] = $secret;
