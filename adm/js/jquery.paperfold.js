...
(function($){
	...
	var init = function(params) {
		...
		//paperfold 총 개수
		data['indexMax'] = params.indexMax;
		
		//paperfold 현재 인덱스
		data['indexNum'] = params.indexNum;
		...
		if (params.toggle) {
			...
			} else if (params.toggle.constructor === String) {
				$toggle = $(params.toggle);
			}
		...
		//$(this).children(".default-content").children(".section").css("height", data['height']);
		...
		data['tops'] = $foldList.find('> .fold > .top01');
		data['bottoms'] = $foldList.find('> .fold > .bottom01');
	...
	var createFold = function(j, topHeight, bottomHeight, data){
        ...
        return $('<div class="fold"/>').css('height',topHeight + bottomHeight).append( 
		        	$('<div class="top01"/>').css('height', topHeight).append(
		        		$('<div class="inner01"/>').css({'top': offsetTop, 'height': data['height']}).append(
		        			data['content'].clone()
		        		)
		        	)).append(
		        	$('<div class="bottom01"/>').css('height', bottomHeight).append(
		        		$('<div class="inner01"/>').css({'bottom': offsetBottom, 'height': data['height']}).append(
		        			data['content'].clone()
		        		)
		        	));
	}
	...
	var methods = {
		toggle: function() {
			...
			if (fallback) {
		        ...
		        if($this.hasClass('visible')){		//visible일 때 fold Open(1)
