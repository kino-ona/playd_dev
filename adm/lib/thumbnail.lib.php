...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    if (!is_dir($target_path)) {
        @mkdir($target_path, Y1_DIR_PERMISSION);
        @chmod($target_path, Y1_DIR_PERMISSION);
    }
    ...
    if($is_large) {
        ...
        if($is_crop) {
            ...
            if ( !((defined('Y1_USE_THUMB_RATIO') && false === Y1_USE_THUMB_RATIO) || (defined('Y1_THEME_USE_THUMB_RATIO') && false === Y1_THEME_USE_THUMB_RATIO)) ){
                if($src_w > $src_h) {
    ...
    if($size[2] == 1) {
        ...
        if(!defined('Y1_THUMB_PNG_COMPRESS'))
            $png_compress = 5;
        else
            $png_compress = Y1_THUMB_PNG_COMPRESS;

        imagepng($dst, $thumb_file, $png_compress);
    } else {
        if(!defined('Y1_THUMB_JPG_QUALITY'))
            $jpg_quality = 90;
        else
            $jpg_quality = Y1_THUMB_JPG_QUALITY;

        imagejpeg($dst, $thumb_file, $jpg_quality);
    }

    chmod($thumb_file, Y1_FILE_PERMISSION); // 추후 삭제를 위하여 파일모드 변경
